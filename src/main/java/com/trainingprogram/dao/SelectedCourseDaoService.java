...
public class SelectedCourseDaoService implements SelectedCourseDao {
	...
	public void insertCourse(CourseRegister Course) throws SQLException {
		...
		try (Connection connection = JDBCUtils.getConnection();
			PreparedStatement preparedStatement = connection.prepareStatement("INSERT INTO `StudentInformation`.`student_register` "
	...
	public CourseRegister selectCourse(int CourseId) {
		...
		try (Connection connection = JDBCUtils.getConnection();
			...
			PreparedStatement preparedStatement = connection.prepareStatement("select * from student_register where id =?");) {
	...
	public List<Course> getAllCourses() {
		...
		return ReadSheetUtils.ReadCourse("D:\\course.xlsx"); // calling excel sheet Path..
	}

	@Override
	public List<String> getAllTime() {

		return ReadSheetUtils.ReadTime("D:\\time.xlsx");
	...
	public String registerAccount(RegisteredAccount ra) throws SQLException {
		...
		String sql1 = "Select * from `StudentInformation`.`registered_account` where email=?";
		...
		String sql2 = "INSERT INTO `StudentInformation`.`registered_account` (`name`, `password`, `email`) VALUES (?, ?, ?)";
	...
	public List<CourseRegister> getEnrolledCourses(String mail) throws SQLException {
		...
		Connection connection = JDBCUtils.getConnection();
		String sql = "select * from student_register where s_email=?;";
		PreparedStatement ps = connection.prepareStatement(sql);
